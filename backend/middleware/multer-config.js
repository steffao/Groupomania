...
const storage = multer.diskStorage({ // objet de config pour multer.
  destination: (req, file, callback) => { // élément précisant le repertoire d'enregistrement
    console.log(req)
    callback(null, './medias'); /// 'image' correspond au répertoire dans lequel on enregistre les images
  },
...
module.exports = multer({storage: storage}).single('media'); // exportation du module avec en argument le storage. single précise que c'est un fichier unique